...
# title.basics: class overview + storage type
...
# title.ratings: class overview + storage type
...
# Function to replace "\N" and "" with NA
to_na <- function(x) {
  y <- as.character(x)
...
```{r Data Exploration}
# checking again the variable type
...
```{r Data Exploration}
# Count ratings below 0 and above 10
sum(title.ratings_clean$averageRating < 0, na.rm = TRUE)
sum(title.ratings_clean$averageRating > 10, na.rm = TRUE)
...
# --- Checking for potential duplicates in cleaned dataset ---

# Check by movie title (since tconst is unique but titles may repeat)
dup_titles <- title.basics_clean %>%
...
# Show first few examples of repeated titles
dup_titles %>% slice_head(n = 20)
