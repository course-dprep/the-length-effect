...
movies_final_clean$decade <- cut(
  movies_final_clean$start_year,
...
ggplot(movies_final_clean, aes(x = runtime_minutes, y = average_rating, color = decade)) +
  geom_point(alpha = 0.2) +
  geom_smooth(method = "lm", se = FALSE, size = 1) +
  labs(
    title = "Runtime and IMDb Ratings by Decade",
    x = "Runtime (minutes)",
    y = "Average IMDb Rating",
    color = "Decade"
  ) +
...
ggplot(movies_final_clean, aes(x = runtime_minutes, y = average_rating)) +
  ...
  labs(
    title = "Runtime–Rating Relationship Across Decades",
    x = "Runtime (minutes)",
    y = "Average IMDb Rating"
  ) +
  theme_minimal(base_size = 9)
...
mod <- lm(average_rating_imp ~ runtime_imp + factor(start_year), data = movies_final_clean)
